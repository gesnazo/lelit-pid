...
#define PCF8574_PIN0 0
#define PCF8574_PIN1 1

#define pinRelayVentil    PCF8574_PIN0
#define pinRelayPumpe     PCF8574_PIN1
...
const long intervaltempmesmax6675 = 500  ;
...
/********************************************************
   K-TYPE SENSOR MAX6675 Y PCF8574
******************************************************/
#if (TEMPSENSOR == 3)
#include <Thermocouple.h>
//#include <MAX6675_Thermocouple_PCF8574.h>
...
RunningAverage myRA(READINGS_NUMBER);
...
void displaymessage(String displaymessagetext, String displaymessagetext2) {
  if (Display == 2) {
    ...
    ******************************************************/
    ...
    display.drawBitmap(41,1, startLogo_bits_coffee,startLogo_width, startLogo_height, WHITE);
    //draw circle
...
void refreshTemp() {
  ...
  ******************************************************/
  ...
  if (TempSensor == 3)
  {
    if (currentMillistemp - previousMillistemp >= intervaltempmesmax6675)
    {
      previousMillistemp = currentMillistemp;
/*
      ...
      {
        yield();
        myRA.addValue(thermocouple->readCelsius());
        delay(DELAY_TIME);
      }
*/
/**/
      //float celsius = myRA.getFastAverage();
      yield();
      float celsius = thermocouple->readCelsius();
      yield();  
      
      if (!checkSensor(celsius) && firstreading == 0) return;  //if sensor data is not valid, abort function
      
      myRA.addValue(celsius);
      DEBUG_println(myRA.getValue(0));
      DEBUG_println(myRA.getValue(1));
      DEBUG_println(myRA.getValue(2));
      Input = myRA.getFastAverage();
      //Input = celsius;
      // Input = random(50,70) ;// test value
      if (Brewdetection == 1)
      {
  ...
  if (OnlyPID == 0) {
    ...
    if (brewswitch > 1000 && bezugsZeit < totalbrewtime && brewcounter >= 1) {
      if (bezugsZeit < preinfusion) {
        ...
        pcf8574.digitalWrite(pinRelayVentil, relayON);
        pcf8574.digitalWrite(pinRelayPumpe, relayON);
      }
      if (bezugsZeit > preinfusion && bezugsZeit < preinfusion + preinfusionpause) {
        ...
        pcf8574.digitalWrite(pinRelayVentil, relayON);
        pcf8574.digitalWrite(pinRelayPumpe, relayOFF);
      }
      ...
      //digitalWrite(pinRelayPumpe, relayOFF);
      pcf8574.digitalWrite(pinRelayVentil, relayOFF);
      pcf8574.digitalWrite(pinRelayPumpe, relayOFF);
    }
...
void setup() {
  ...
  ******************************************************/
  ...
  pcf8574.pinMode(pinRelayVentil, OUTPUT);
  pcf8574.pinMode(pinRelayPumpe, OUTPUT);
  pcf8574.begin();
  ...
  ******************************************************/
  ...
  #if (TEMPSENSOR == 3)
  if (TempSensor == 3) {
    DEBUG_println("TempSensor = 3");
    //thermocouple = new MAX6675_Thermocouple_PCF8574(SCK_PIN, CS_PIN, SO_PIN);
    thermocouple = new MAX6675_Thermocouple(SCK_PIN, CS_PIN, SO_PIN);
/* 
    Thermocouple* originThermocouple = new MAX6675_Thermocouple_PCF8574(SCK_PIN, CS_PIN, SO_PIN);
